...
/**
 * Every history record re-keyed (or decrypted), as puts for the vault's single
 * atomic transaction; nothing is written here.
 */
export async function prepareHistoryRewrite(
  from: CryptoKey | null,
  to: CryptoKey | null,
): Promise<{ ops: WriteOp[]; bytes: number }> {
  const ops: WriteOp[] = [];
  let bytes = 0;
  for (const record of await idb.getAll<StoredHistory>('history')) {
    const value = to ? await seal(to, await open(record, from)) : await open(record, from);
    bytes += JSON.stringify(value).length * 2;
    ops.push({ store: 'history', op: 'put', value });
  }
  return { ops, bytes };
}
